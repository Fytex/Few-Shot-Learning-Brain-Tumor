...
def make_triples(x, y):
    ...
    return np.array(triples), np.array(labels).astype("float32")
...
def load_images(basedir, path, input_size, labels=None):
    ...
    if isinstance(labels, dict):
        for k, v in batches.class_indices.items():
            labels[v] = k
